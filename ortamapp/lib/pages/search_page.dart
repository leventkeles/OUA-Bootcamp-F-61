...
class _SearchPageState extends State<SearchPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Grup Ara", style: TextStyle(fontSize: 27, fontWeight: FontWeight.bold, color: Colors.white)),
      ),
      body:
      Column(
        children: [
  ...
  initiateSearchMethod() async {
    String results = searchController.text;
    if (searchController.text.isNotEmpty) {
      ...
      await DataBaseService().searchByDomain(results.toLowerCase(), userEmailDomain).then((snapshot) {
        setState(() {
